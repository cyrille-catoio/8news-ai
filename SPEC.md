...
## 17. Changelog (v1.49 → v1.90)

Summary table (one line per release). **§17.1** expands **v1.65–v1.90** in detail (aligned with `src/lib/changelog-entries.ts` and current code).

| Version | Key Changes |
|---|---|
| v1.90 | Auto-sync changelog from code to DB: entries defined in `src/lib/changelog-entries.ts`, missing versions inserted automatically on first `GET /api/changelog` call after deploy. No more manual SQL needed for new releases. |
...
### 17.1 Release detail — v1.65 through v1.90

| Ver. | EN (what shipped) | FR (titre seed migration) |
|------|-------------------|----------------------------|
| **1.90** | Changelog entries are now defined in **`src/lib/changelog-entries.ts`** (TypeScript array). `GET /api/changelog` auto-inserts any missing versions into the `changelog` table on first call after deploy (one-time `SELECT` + `INSERT` of missing rows, then cached for the process lifetime). Eliminates the need for manual `insert-changelog-*.sql` execution in Supabase SQL Editor after each release. | *Auto-sync changelog code → DB, plus de SQL manuel* |
...
> **Note:** From **v1.90+**, changelog entries are auto-synced from `src/lib/changelog-entries.ts` to the DB on first `GET /api/changelog` call. For older versions missing from the DB, the auto-sync will insert them automatically. Manual `insert-changelog-*.sql` files are no longer needed for new releases. **SPEC** and **runtime** remain authoritative when copy diverges.
