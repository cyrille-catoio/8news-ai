...
- **v2.6.5+** `gpt-5.5` — daily Top articles AI summary cron (`generate-top-summary.ts` → `cron-top-summary-background`). Editorial flagship model; the snapshot is read by /top-articles and the home `Top24hHero` accordion via `GET /api/news/top-summary/latest` (no on-demand LLM call from any user-facing surface). **v2.20+** only the EN edition runs the full gpt-5.5 editorial pass (single source of truth for groups + importance scores); the FR edition is a translation of the EN pivot bullets on `gpt-4.1-mini` (`OPENAI_MODELS.topSummaryTranslate`) with structure/refs/scores preserved verbatim — identical scores across langs, with native FR generation kept as fallback (pivot missing or translation failure).
...
│       ├── generate-top-summary.ts     # Daily Top 24h snapshot (`gpt-5.5`) — **v2.13.5+** `selectTopArticleBullets()` caps persisted bullets at 8 (2 pinned videos + 6 articles); **v2.20+** FR edition = translation of the EN pivot (`translateTopSummaryBullets`, gpt-4.1-mini) with identical groups/scores, native FR fallback
...
│       ├── cron-top-summary-background.ts      # Daily Top 50 AI summary snapshot (gpt-5.5 EN pivot; **v2.20+** FR = translation of the EN bullets), persisted into `top_summaries`. Reads served by GET /api/news/top-summary/latest — no on-demand LLM call from /top-articles anymore.
...
**SPA + shared**: `AppHeader` (**v2.5.17+** mounts the `CryptoTicker` on every page except `currentPage === "landing"`), `CryptoTicker` (**v2.5.17+** live top-50 CoinGecko ticker — see §19), `CryptoTickerSettingsPage` (settings-page section, search + max-12 checkbox selection), `GeneralMenu` (+ `SeoGeneralMenu`; **v2.20+** both render from the shared `GENERAL_MENU_ITEMS` list so the SPA and SSR menus can't drift, with a new « Shorts » pill at `/app/shorts`), `SeoNavBar` (**v2.5.3+** intercepts language toggle to persist `preferred_lang`), `AuthModal`, `BriefingPage` (the SPA's default landing — **v2.6.6** order: `Top24hHero` → TOP VIDEO → Top story → All transcribed videos → Trending strip → daily summary teaser → Top 5 → Your topics → Footer CTAs; **v2.20.5+** the TOP VIDEO + TOP STORY cards moved to the dedicated `TopStoriesPage`), **`TopStoriesPage`** (**v2.20.5+** `/app/top-stories` — hosts the TOP VIDEO + TOP STORY cards verbatim: 10-min bucket sync, ‹ › history chevrons over `home_surface_queue`, top-feed fallback for the article card), **`Top24hHero`** (v2.6.6 — gold accordion card pinned at the top of the home, reads `GET /api/news/top-summary/latest`, shows group titles only and expands sub-bullets on click; **v2.20+** heading row carries the gold « Ask the AI » + « Fullscreen » pills, the latter opening the immersive `PodcastReader`), `TopFeedSection`, `SummaryBox` (v2.6.5+ renders an optional `bullet.title` in gold above each bullet body, groups consecutive same-title rows), `AllArticlesTab`, `StatsPage`, `CronMonitorPage`, `TopicsPage/`, `FeedsAdminPage`, `CategoriesPage`, `FavoritesPage`, `FavoriteButton`, `CopyLinkButton`, `ScoreMeter`, `ChangelogPage`, `SettingsPage` (`MyAccountSection`, `UsersSection`, `VoiceAccordion`, crypto ticker top-50 selector), `AudioPlayer`, `TopicPersonalizationBar`, `TopicOnboardingModal`, `SummariesBrowsePage`.

**Video surface**: `VideosPage` (today / day-by-day video list with Shorts toggle), `VideoCard` (iframe embed with **v2.x+** localhost-aware `youtube-nocookie` swap to fix black-screen; **v2.13.5+** every displayed title/description/aria-label/TTS intro goes through `stripEmojis()` from `video-card/VideoCardHelpers.ts` — raw titles stay untouched in DB and API payloads), `VideoPageAudio`, `DownloadTranscriptButton`, **`ShortsPage`** (**v2.20+** fullscreen TikTok-style vertical Shorts feed at `/app/shorts` — scroll-snap slides, active-slide-only iframe, reads `GET /api/videos/shorts`; pure helpers in `shorts/ShortsHelpers.ts`).

**Helper subfolders (v2.13.4 dedup)**: `briefing/` (home sections + `styles.kicker()` + the single `HistoryArrows` component shared by TOP STORY / TOP VIDEO / Top 24h chevrons), `top24h/` (`Top24hHeroHelpers.ts` — `groupBullets`/`countGroups`, `top24h-cache.ts`; **v2.20+** `PodcastReader.tsx` + `PodcastReaderHelpers.ts` — fullscreen one-news-per-slide Daily Podcast reader, vertical scroll-snap swipe + Previous/Next buttons + keyboard), `video-card/` (`VideoCardHelpers.ts` — `stripEmojis`, `extractVideoBulletText`), `app-shell/` (**v2.20+** incl. `MyTopicsPage`), `shorts/` (**v2.20+** `ShortsHelpers.ts`), `podcast-chat/` (**v2.13+** `DailyPodcastChatPanel` + `PodcastChatMarkdown`), and `video-markdown.tsx` (**v2.13.4+** single react-markdown component map with a `card`/`page` size variant — replaces the former `VideoCardMarkdown.tsx` + `video-page-markdown.tsx` duplicates). Pure helpers in these folders carry colocated `__tests__/` vitest suites.
...
│   └── shorts/route.ts           # **v2.20+** GET — Shorts feed (< 180 s, 5-day window, duration backfill) for `ShortsPage`
├── archives/
...
├── test-score/route.ts           # GET — manual scoring replay/debug for one topic (`scoreTopicForAdmin`; CRON_SECRET, `?topic=` required, `?hours=`/`?limit=`/`?debug=1`)
...
- Pipeline per lang:
  ...
  - **v2.20+ EN pivot → FR translation.** Only the EN edition runs the gpt-5.5 editorial pass; its uncapped bullets are handed back to the FR run (`pivotBullets`), which translates titles + texts on `gpt-4.1-mini` (`translateTopSummaryBullets`) with structure, refs and `importance` preserved verbatim — so the two langs always show the same groups and scores, and the FR call no longer ships the 50 articles. The top-summary prompt also dropped the unused `relevant` array (50 rewritten snippets of pure output-token waste per call). FR-only runs (watchdog self-heal `?langs=fr`) and translation failures fall back to the previous native FR generation.
...
| `/api/videos/shorts` | GET | **v2.20+** Feed for the « Shorts » page (`ShortsPage.tsx`, TikTok-style vertical player behind the general-menu pill at `/app/shorts`). Returns `{ shorts: VideoItem[] }` — the YouTube Shorts (`duration_sec` < 180) from `youtube_videos` over the last 5 local days (`?since=` ISO lower bound sent by the SPA, `?days=` fallback window, max 14), newest first. Shorts are filtered in SQL; unknown-duration rows are fetched separately (≤ 150) and backfilled via `enrichDurations` before merging. No RSS refresh in-route — freshness rides on the 15-min transcribe cron. |
...
The SPA at `/app` has 15+ pseudo-pages managed by `currentPage` state (`"briefing"` | `"topStories"` (**v2.20.5+**, `/app/top-stories`) | `"home"` (= Top 50) | `"stats"` | `"crons"` | `"topics"` | `"feeds"` | `"categories"` | `"dailySummaries"` | `"favorites"` | `"topArticles"` | `"summaries"` | `"videos"` | `"youtubeChannels"` | `"changelog"` | `"settings"`). Route-mapped via `next.config.ts` rewrites for hard-refresh resilience. `topics`, `feeds`, `categories`, `dailySummaries`, `youtubeChannels` are owner-only. `favorites` requires any authenticated user.
...
- **v2.20.5+** New « **Top Stories** / **À la une** » pill (`topStoriesBtn`) pointing at `/app/top-stories`; **v2.20.6+** « Shorts » moved up to second position (right of « Today »), so the row starts Today · Shorts · Top Stories — the dedicated page hosting the TOP VIDEO + TOP STORY cards moved off the home briefing (shared `GENERAL_MENU_ITEMS`, so it appears on both the SPA and SSR menus)
...
**Default page**: `BriefingPage` — a composite landing inside the SPA. **v2.20.5+** the TOP VIDEO · maintenant and TOP STORY · maintenant cards (items 2-3 below) moved unchanged to the dedicated `/app/top-stories` page (`TopStoriesPage.tsx`, general-menu pill « Top Stories / À la une »), and the podcast hero renders always expanded (`defaultOpen`, the per-day « Lu / Read » checkbox was removed). **v2.20.6+** the Trending strip moved to the Topics page (`/app/articles`, above the topic list, `TrendingStripSection`); the « Today's Briefing · Top 5 » section was removed outright (its « N new since your visit » badge machinery with it — the full ranking lives on `/app/top-articles`); and the daily-summary teaser moved up right below the podcast hero with a 2× longer excerpt (6 bullets / 840 chars / 10-line clamp). Home order is now: Daily Podcast → daily-summary teaser → newsletter CTA → Your topics → transcribed videos → footer CTAs. Historical vertical order (top → bottom):
...
- **v2.20** — Fullscreen reading surfaces + homogenized Daily Podcast editions. New « Shorts » SPA page at `/app/shorts` (`ShortsPage` + `GET /api/videos/shorts`, TikTok-style vertical snap feed of < 180 s videos, general-menu pill shared SPA/SSR via `GENERAL_MENU_ITEMS`). Fullscreen Daily Podcast reader (`top24h/PodcastReader.tsx`, one news per slide, swipe + buttons + keyboard) behind a gold « Fullscreen » pill in the hero heading row, next to the relocated « Ask the AI » pill. Daily Podcast FR edition now = translation of the EN pivot (identical groups/importance scores across langs, `gpt-4.1-mini`, native FR fallback); unused `relevant` array dropped from the top-summary prompt. New owner debug route `GET /api/test-score`. No DB migration.
