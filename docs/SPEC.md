...
- **v2.20.5+** New « **Top Stories** / **À la une** » pill (`topStoriesBtn`) right after « Today », pointing at `/app/top-stories` — the dedicated page hosting the TOP VIDEO + TOP STORY cards moved off the home briefing (shared `GENERAL_MENU_ITEMS`, so it appears on both the SPA and SSR menus)
...
**Default page**: `BriefingPage` — a composite landing inside the SPA. **v2.20.5+** the TOP VIDEO · maintenant and TOP STORY · maintenant cards (items 2-3 below) moved unchanged to the dedicated `/app/top-stories` page (`TopStoriesPage.tsx`, general-menu pill « Top Stories / À la une ») — the home now flows straight from the podcast hero into Top 5 / Trending, and the podcast hero renders always expanded (`defaultOpen`, the per-day « Lu / Read » checkbox was removed). Historical vertical order (top → bottom):
