...
export default async () => {
  ...
  for (const topic of batch) {
    ...
    if (inserted > 0 && budgetRemaining() > SCORE_CALL_RESERVE_MS + CRON_SAFETY_RESERVE_MS) {
      ...
      const capacityFactor = clamp((remaining - SCORE_CALL_RESERVE_MS) / 4_000, 0.5, 1.6);
  ...
  lines.push(
    `[run] cron=fetch topics_total=${n} topics_target=${batch.length} topics_processed=${processedTopics} inserted=${totalInserted} mini_scored=${totalMiniScored} deadline_stops=${deadlineStops} elapsed_ms=${Date.now() - startedAt} budget_ms=${Math.min(CRON_WALL_MS, CRON_BUDGET_MS)}`,
  );
