 ...
 *
 * The current ElevenLabs model accepts much larger prompts, but this
 * endpoint is still a synchronous serverless request that must generate
 * and return the whole MP3 before the function budget expires. Keep the
 * cap at the known-playable range until narration is chunked or moved to
 * an async job; otherwise busy briefings can time out and produce no audio
 * at all.
 *
